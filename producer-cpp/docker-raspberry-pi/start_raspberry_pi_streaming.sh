...
omxh264enc control-rate=2 target-bitrate=512000 \
inline-header=FALSE periodicty-idr=20 ! h264parse ! \
video/x-h264,stream-format=avc,alignment=au,width=640,height=480,framerate=30/1,profile=baseline ! \
kvssink stream-name=$STREAM_NAME access-key=$AWS_ACCESS_KEY_ID secret-key=$AWS_SECRET_ACCESS_KEY
